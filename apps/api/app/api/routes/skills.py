...
async def list_skills(
    member_id: UUID, x: SkillService = Depends(service)
...
async def delete_skill(id: UUID, x: SkillService = Depends(service)) -> Response:
    x.delete(id)
    return Response(status_code=204)


@router.get("/api/v1/member-skills/{id}/evidences", response_model=list[EvidenceResponse])
async def evidences(id: UUID, x: SkillService = Depends(service)) -> list[EvidenceResponse]:
